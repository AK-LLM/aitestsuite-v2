...
def run(scenario, endpoint, apikey, mode, **kwargs):
    ...
    return {
        "risk_level": "High Risk" if errors > 3 else "Medium Risk" if errors > 0 else "Low Risk",
        "issue": f"{errors} errors in {total} rapid requests, avg latency {avg_latency:.2f}s",
        "recommendation": "Implement API rate limits, review SRE capacity for model serving."
    }
